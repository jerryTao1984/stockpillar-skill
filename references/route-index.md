...
- `GET /stocks/{ts_code}/prices/kline`: Auth `Bearer token`; required params path `ts_code`, `start_date`, `end_date`; optional `limit`; optional HK/US `freq=day|week|month|1m|5m|60m` and `adjust=qfq|hfq|none`. A-share reads the A-share market-data service; HK/US without `freq` reads stored ODS daily K-line; HK/US with `freq` reads Futu OpenD history and accepts suffix or Futu prefix code forms.
...
- `GET /stocks/{ts_code}/prices/minute`: Auth `Bearer token`; required params path `ts_code`, query `trade_date`; optional query `freq` (only `1m`); use for same-day 1m minute bars. A-share rows are queried live from QMT Bridge `/market/minute`; HK/US rows use Futu OpenD and accept suffix or Futu prefix code forms. For HK/US historical minute bars, use `/stocks/{ts_code}/prices/kline?freq=1m|5m|60m`.
