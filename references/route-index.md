...
- `GET /stocks/{ts_code}/prices/kline`: Auth `Bearer token`; required params path `ts_code`, `start_date`, `end_date`; optional `limit`; optional HK/US `freq=day|week|month|1m|5m|60m` and `adjust=qfq|hfq|none`. A-share reads the A-share market-data service; HK/US without `freq` reads stored ODS daily K-line; HK/US with `freq` reads StockPillar's server-side HK/US history channel and accepts suffix or market-prefix code forms.
- `GET /prices/realtime`: Auth `Bearer token`; required params `ts_codes`; use for one or more realtime quotes. A-share codes use `600519.SH`/`000001.SZ`; Hong Kong and U.S. codes may use either StockPillar suffix form (`00700.HK`, `AAPL.US`) or market-prefix form (`HK.00700`, `US.AAPL`). HK/US rows are read through StockPillar's server-side realtime channel when available; provider/source fields are internal implementation details.
- `GET /stocks/{ts_code}/prices/minute`: Auth `Bearer token`; required params path `ts_code`, query `trade_date`; optional query `freq` (only `1m`); use for same-day 1m minute bars. A-share rows are queried live from StockPillar's A-share minute-bar channel; HK/US rows use StockPillar's server-side HK/US minute-bar channel and accept suffix or market-prefix code forms. For HK/US historical minute bars, use `/stocks/{ts_code}/prices/kline?freq=1m|5m|60m`.
- `GET /stocks/{ts_code}/prices/ticks`: Auth `Bearer token`; scope `market:read`; required params path `ts_code`, query `trade_date` (current day only); optional `start_time` (default `091500`) and `end_time` (default `092500`). Use for one A-share's same-day QMT Level-1 tick replay, especially the call auction; the window is limited to 30 minutes and does not read OSS.
- `GET /market/premarket-panel`: Auth `Bearer token`; required params `none`; optional query `codes` comma-separated. Use for the preset small-cap premarket panel plus optional extra tickers; response data includes `items`, `missing_codes`, and `panel_codes`.
